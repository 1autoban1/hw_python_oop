...
class Running(Training):
    ...
    def get_spent_calories(self) -> float:
        return ((self.Coeff_callorie_1*self.get_mean_speed()-self.Coeff_callorie_2)*self.weight/self.M_IN_KM*self.duration*self.MIN_IN_HR)
...
class SportsWalking(Training):
    ...
    def get_spent_calories(self) -> float:
        return ((self.Coeff_calorie_1*self.weight+(self.get_mean_speed()**2//self.height)*self.Coeff_calorie_2*self.weight)*self.duration*self.MIN_IN_HR)
...
Read_workout_type: dict = {'SWM': Swimming, 'RUN': Running, 'WLK': SportsWalking}
